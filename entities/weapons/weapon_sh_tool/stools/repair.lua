...
if CLIENT then TOOL.SelectIcon = surface.GetTextureID( "toolicons/repair" ) end
		...
		for _, v in ipairs(ents.FindInSphere(ownerpos,200)) do
			if IsValid( v ) and !v:IsPlayer() then
				...
				if v:GetMaxHealth() > 1 then
					...
					if HEALTH_NUM:GetBool() then draw.SimpleTextOutlined( math.floor(v:Health()).."/"..v:GetMaxHealth(), "UiBold", pos.x, pos.y+16, Color(255,255,255,255), TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 1, Color(0,0,0,255) ) end
				end
			end
		end
	end
