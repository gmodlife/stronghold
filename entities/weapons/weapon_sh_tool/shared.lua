...
function SWEP:InitializeTools()
	...
	for k, v in pairs(self.Tool) do
		tbl[k] = table.Copy(v)
		tbl[k].SWEP = self
		tbl[k].Owner = self.Owner
		tbl[k].Weapon = self.Weapon
		tbl[k].RadialAngle = i * self.ToolAngBetween - math.pi/2
		while tbl[k].RadialAngle < 0 do tbl[k].RadialAngle = tbl[k].RadialAngle + math.pi*2 end
		if tbl[k].Initialize then tbl[k]:Initialize() end
		i = i + 1
	end
